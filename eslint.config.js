      ...
      repolocal: {
        rules: {
          'no-readonly': {
            ...
            create(context) {
              return {
                'TSPropertySignature[readonly=true],TSTypeOperator[operator="readonly"],PropertyDefinition[readonly=true]':
                  (node) => {
                    context.report({
                      message: "Don't use `readonly`",
                      node,
                      fix: (fixer) =>
                        fixer.replaceText(node, context.sourceCode.getText(node).replace(/^readonly /, '')),
                    });
                  },
              };
            },
