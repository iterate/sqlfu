...
Done. Generated query wrappers now include a per-file SHA-256 source hash comment, and the lint plugin reports missing, hashless, or stale wrappers when linting `.sql` files. Docs, focused tests, fixtures, typecheck, and the full `sqlfu` package test suite are complete.

## Checklist

- [x] Add a generated wrapper metadata comment containing a full SHA-256 hex hash of the exact source `.sql` file bytes. *Implemented in `packages/sqlfu/src/typegen/index.ts` via the generated wrapper header.*
...
- [x] Report a clear lint error when the generated wrapper hash is missing or differs from the current source hash. *Covered by missing-metadata and stale-hash lint tests.*
...
- Regenerated generate snapshot fixtures after wrapper headers changed.
- Verification:
