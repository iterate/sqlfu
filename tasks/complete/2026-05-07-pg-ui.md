...
Done in PR #89, merged 2026-05-07. The sqlfu UI now works against a postgres project for the schema/drift/draft/migrate flows covered by the PR's pg UI specs. This task was stacked off `pg-package`; remaining unchecked plan text below was the original TDD path and is now represented by the implementation notes.
...
- [x] **Scaffold the pg template project.** _Landed in PR #89 as `packages/ui/test/template-project-pg/` and pg-specific fixtures._
- [x] **Write the first failing spec.** _Landed in PR #89; the first pg studio smoke surfaced sqlite-specific schema reads._
- [x] **Iterate**: each red is one of:
  ...
  _PR #89 made the exercised UI routes dialect-aware where needed._
- [x] **Repeat for the other golden paths**: query runner, drift, sync, migration apply. _Covered by the pg UI specs shipped in PR #89._
- [x] **Final pass**: run the full playwright suite. _Verification ran in PR #89; no stale root task remains._
