...
Implementation complete. The Bun adapter now normalizes bare named records before calling `bun:sqlite` prepared statements; the focused real-Bun regression, typecheck, changed-file lint, and changed-file format checks pass. The full `sqlfu` package test suite was attempted but still has unrelated worktree-environment failures in watch/UI/better-auth tests.
