...
- [x] `website/src/pages/index.astro` + `website/src/styles/landing.css`: refine the Batteries showcase. _panels now keep a fixed 358px stage; CLI is first; admin UI prints `sqlfu ready at https://sqlfu.dev/ui` and uses the latest clipboard-cropped UI screenshot from `website/public/assets/landing/sqlfu-ui-crop.png`; tracing shows a small Hono route plus a trace rendering; Outbox is replaced by CLI examples; Agent skill drops `@latest`; Formatter demos `npx sqlfu format`_
