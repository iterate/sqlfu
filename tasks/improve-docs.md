...
- [x] `website/src/pages/index.astro`: simplify the schema demo to a nullable `published integer` add and animate the generated TypeScript file. _keeps the migration example in the simple SQLite-safe nullable-column case; `src/posts.ts` now types in quickly after focused `sqlfu generate` output that lists only `./.generated/get-posts.sql.ts`_
