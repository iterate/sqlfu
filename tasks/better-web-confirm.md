...
instead we should use a websocket connection so the server can just send the `confirm` params to the client which can render, get the user's confirmation, and send the result back to the server (which might need to hold on to a generated id)
