...
import {sql, type Queryable} from '../pgkit-compat.js'
// Removed `createClient` import (was used to convert a connection-string
// argument to `PostgreSQL.create` into a client). Callers must pass a
// Queryable directly under the sqlfu vendor; see runtime check below.
import {AutoThisAssigner} from '../auto-this.js'
import {TopologicalSorter} from '../graphlib/index.js'
import {ColumnInfo, Inspected, BaseInspectedSelectable, TableRelated, pick} from '../inspected.js'
import {DBInspector} from '../inspector.js'
import {asa, isa} from '../isa-asa.js'
import {quoted_identifier, quotify} from '../misc.js'
import {queries as queryContents} from '../queries.js'
import {Queries} from '../types.js'
import {groupBy, isEqual} from '../util.js'
...
export class PostgreSQL extends DBInspector {
  ...
  // Sqlfu vendor: connection-string overload removed. Callers must pass a
  // Queryable. Adapt sqlfu's AsyncClient via `adaptAsyncClient` from
  // `../pgkit-compat.js`.
  static async create(connection: Queryable, props?: PostgreSQLOptions): Promise<PostgreSQL> {
