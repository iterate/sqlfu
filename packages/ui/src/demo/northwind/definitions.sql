...
sum((order_details.unit_price*quantity*(1-discount)/100)*100) as subtotal
from order_details
...
where orders.shipped_date between datetime('1997-01-01') and datetime('1997-12-31')
...
from customers 
     join orders on customers.customer_id = orders.customer_id
where orders.order_date BETWEEN datetime('1997-01-01') and datetime('1997-12-31');

create view sales_totals_by_amount as
select order_subtotals.subtotal as sale_amount, 
                  orders.order_id, 
               customers.company_name, 
                  orders.shipped_date
from customers 
...
and (orders.shipped_date BETWEEN datetime('1997-01-01') and datetime('1997-12-31'));
...
select     product_sales_for_1997.category_name, 
      ...
      on categories.category_id = products.category_id
       join order_details_extended 
         on products.product_id = order_details_extended.product_id                
           join orders 
             on orders.order_id = order_details_extended.order_id 
where orders.order_date BETWEEN datetime('1997-01-01') and datetime('1997-12-31')
