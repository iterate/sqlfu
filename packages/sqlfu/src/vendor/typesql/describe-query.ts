import type { SchemaDef, ParameterDef, TypeSqlError, PreprocessedSql, MySqlDialect, NamedParamInfo } from './types.js';
import { extractQueryInfo } from './mysql-query-analyzer/parse.js';
import {type Either, left} from '../small-utils.js';
...
export async function parseSql(client: MySqlDialect, sql: string): Promise<Either<TypeSqlError, SchemaDef>> {
	...
	return left({
		...
		description: 'Vendored TypeSQL is sqlite-only in sqlfu for now.',
	});
}
