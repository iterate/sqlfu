import { SchemaDef, TsFieldDescriptor, ParameterDef, MySqlDialect, TypeSqlError, TsParameterDescriptor } from '../types.js';
import { type Either, isLeft, right } from '../../small-utils.js';
import { mapper, type MySqlType } from '../mysql-mapping.js';
import { parseSql } from '../describe-query.js';
	...
	writer.write(`export async function ${camelCaseName}(${functionArguments}): Promise<${functionReturnType}>`).block(() => {
		if (tsDescriptor.dynamicQuery == null) {
			...
			tsDescriptor.dynamicQuery.select.forEach((fragment) => {
				writer
					.write(
						`if (params?.select == null || ${fragment.dependOnFields.map((fieldIndex) => `params.select.${tsDescriptor.columns[fieldIndex].name}`).join('&&')})`
					)
					.block(() => {
...
function generateDynamicQueryFrom(
	writer: CodeBlockWriter,
	sqlVar: string,
	fragment: FragmentInfoResult,
