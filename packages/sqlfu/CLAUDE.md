...
- **Naive merge into one `tsconfig.build.json` with `noCheck: true`.** `noCheck: true` distorts inferred types in our own code — `src/api/internal.ts`'s `getSchemaAuthorities` return type loses `string | null` fields (observed 2026-04-20 during the collapse investigation). Anything consuming the emitted API declaration files gets a subtly different public surface.
