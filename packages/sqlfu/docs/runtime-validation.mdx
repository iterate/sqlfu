...
The generated wrapper uses the schemas itself instead of only re-exporting them for consumers. That is the value over plain TS types: every query gets a validated request/response contract by default. The exported schemas are also usable for forms (`@rjsf`, react-hook-form), tRPC inputs, RPC wire validation, fixtures, etc., but that is a secondary benefit.
...
export const findPostBySlug = Object.assign(
  async function findPostBySlug(
    ...
    if ('issues' in parsedParamsResult)
      throw new Error(prettifyStandardSchemaError(parsedParamsResult) || 'Validation failed');
...
Namespace merging is what makes `findPostBySlug.Params` resolve as a value (the schema) _and_ a type. Consumers don't have to think about this: they write `findPostBySlug.Params` in either position.
...
The generated file is readable and small. If you want a _specific_ validator
refinement (e.g. `.url()`, `.email()`, custom refinements) for a column, wrap
the generated function in your application code:

```ts
import {findPostBySlug as rawFindPostBySlug} from './sql/.generated/find-post-by-slug.sql.ts';
import {z} from 'zod';
...
us. Column-level refinement is an application concern. Pluggable validators and
per-column overrides are tracked in
[`tasks/typegen-extensibility.md`](https://github.com/iterate/sqlfu/blob/main/tasks/typegen-extensibility.md).
