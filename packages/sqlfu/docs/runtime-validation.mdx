...
Generated wrappers can be emitted with a validator library — [arktype](https://arktype.io), [valibot](https://valibot.dev), [zod](https://zod.dev), or [zod/mini](https://zod.dev/v4#introducing-zod-mini) — as the source of truth. Params are validated on the way in, rows are validated on the way out, and types are derived via the validator's native inference helper. One definition per thing, no drift.
...
export default {
  ...
  queries: './sql',
  generate: {
    validator: 'zod', // or 'arktype' | 'valibot' | 'zod-mini'
  },
};
```
...
Whatever you pick, the public shape of the generated wrapper — one callable with `.Params`, `.Result`, `.sql` attached — is identical. You can switch later without touching callsites.
...
The public shape — one callable, `.Params`, `.Result`, `.sql` — is identical across every validator. You can swap `generate.validator` and regenerate without touching any callsites.
