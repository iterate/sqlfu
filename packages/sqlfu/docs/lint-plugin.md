...
Flags checked-in `.sql` query files whose generated wrapper is missing or stale.
...
The rule compares the current `.sql` file with the `sqlfu-source-hash` metadata comment emitted in the generated wrapper. If the wrapper is missing, the metadata is missing, or the hash no longer matches, lint reports that `sqlfu generate` needs to be run.
