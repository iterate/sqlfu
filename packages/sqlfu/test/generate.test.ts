...
test('generate emits trivial wrappers for drop / alter / pragma / multi-statement / comments', async () => {
  // Covers the ANTLR-driven DDL branches that the old `isDdlStatement` regex either only
  ...
  // Each one should compile as a plain client.run(sql) wrapper.
  for (const file of [
  ...
  // Every DDL file is skipped by the query catalog (same contract as the pre-existing
  // `create table` DDL test).
  expect(await project.readJson('.sqlfu/query-catalog.json')).toMatchObject({queries: []});
  await expect(project.getCompileDiagnostics()).resolves.toEqual([]);
});
