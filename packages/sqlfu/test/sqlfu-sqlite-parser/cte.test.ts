...
test('CTE with column alias list', () => {
  const stmt = parseSelectStmt(`with t(a, b) as (select id, slug from posts) select a, b from t`);
  expect(stmt.with_clause).toMatchObject({
...
test('multiple CTEs', () => {
  const stmt = parseSelectStmt(`with a as (select 1), b as (select 2) select * from a union all select * from b`);
  expect(stmt.with_clause).toMatchObject({
    recursive: false,
    ctes: [{name: 'a'}, {name: 'b'}],
  });
