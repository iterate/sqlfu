...
test('INNER JOIN with ON constraint', () => {
  const from = parseSelectStmt(`select p.id from posts p inner join authors a on p.author_id = a.id`).select_cores[0]
    .from;
...
test('three-way join chain', () => {
  const from = parseSelectStmt(`select 1 from a inner join b on a.x = b.x left join c on b.y = c.y`).select_cores[0]
    .from;
  expect(from).toMatchObject({
    kind: 'JoinChain',
    chain: {
