...
import type {Client, SqlfuMigrationPrefix, SqlfuProjectConfig} from '../../src/types.js';
...
export async function createMigrationsFixture(
  ...
  const projectConfig: SqlfuProjectConfig = {
    projectRoot: root,
    db: dbPath,
    migrations: {path: path.join(root, 'migrations'), prefix: migrationPrefix, preset: 'sqlfu'},
