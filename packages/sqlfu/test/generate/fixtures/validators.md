Validator-integration fixtures: zod, valibot, zod/mini, arktype, typebox. Runtime-behaviour tests
...
## validator: typebox emits native TypeBox schemas
...
```ts (sql/.generated/find-post-by-slug.sql.ts)
import {type Client, parseTypeBox} from 'sqlfu';
...
const ParamsValidator = Schema.Compile(Params);
...
const ResultValidator = Schema.Compile(Result);
const sql = `select id, slug, title, status from posts where slug = ? limit 1;`;
const query = (params: findPostBySlug.Params) => ({
	name: "findPostBySlug",
	sql,
...
export const findPostBySlug = Object.assign(
	async function findPostBySlug(client: Client, params: findPostBySlug.Params): Promise<findPostBySlug.Result | null> {
		const rows = await client.all(query(parseTypeBox(ParamsValidator, params)));
		if (rows.length === 0) return null;
		return parseTypeBox(ResultValidator, rows[0]);
	},
	{ Params, Result, sql, query },
);

export namespace findPostBySlug {
	export type Params = Type.Static<typeof findPostBySlug.Params>;
	export type Result = Type.Static<typeof findPostBySlug.Result>;
}
```
...
## validator: typebox + prettyErrors: false lets raw ParseError propagate

<details data-outputs="sql/.generated/find-post-by-slug.sql.ts,sql/.generated/index.ts,sql/.generated/tables.ts">
<summary>input</summary>
...
export default {
  ...
  definitions: './definitions.sql',
  queries: './sql',
  generate: {validator: 'typebox', prettyErrors: false},
};
```
...
export const findPostBySlug = Object.assign(
	async function findPostBySlug(client: Client, params: findPostBySlug.Params): Promise<findPostBySlug.Result | null> {
		const rows = await client.all(query(ParamsValidator.Parse(params)));
		return rows.length > 0 ? ResultValidator.Parse(rows[0]) : null;
	},
	{ Params, Result, sql, query },
);
...
const query = { name: "latestPost", sql, args: [] };
...
const query = (params: insertPost.Params) => ({ name: "insertPost", sql, args: [params.slug] });
