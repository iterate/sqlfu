...
const query = (params: insertPosts.Params) => {
	...
	const expandedSql = `insert into posts (slug, title) values ${params.posts.map(() => "(?, ?)").join(', ')} returning id, slug, title;`;
	return { sql: expandedSql, args: [...params.posts.flatMap((item) => [item.slug, item.title])], name: "insertPosts" };
};

export const insertPosts = Object.assign(
	async function insertPosts(client: Client, params: insertPosts.Params): Promise<insertPosts.Result> {
		const rows = await client.all<insertPosts.Result>(query(params));
		return rows[0];
	},
	{ sql, query },
);
