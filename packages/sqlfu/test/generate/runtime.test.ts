...
test('generated query wrappers accept an Effect client', async () => {
  await using project = await createRuntimeFixture({
    definitionsSql: `create table posts (id integer primary key, slug text not null, title text);`,
    files: {
      'sql/list-posts.sql': `select id, slug, title from posts order by id;`,
      'sql/find-post-by-slug.sql': `select id, slug, title from posts where slug = :slug limit 1;`,
    },
    config: {generate: {effect: true}},
  });

  await project.generate();
  await project.applyStatements(`insert into posts (id, slug, title) values (1, 'hello', 'Hello');`);

  const effectMod = await project.importTranspiledModule<any>('sql/.generated/effect.ts');
  const queriesMod = await project.importTranspiledModule<any>('sql/.generated/queries.ts');

  using database = project.openDatabase();
  const client = createNodeSqliteClient(database.database);
  const DB = effectMod.SqlfuClient.make().pipe(Effect.provide(effectMod.SqlfuClient.DefaultServices(client)));

  const program = Effect.gen(function*() {
    const db = yield* DB;
    const posts = yield* queriesMod.listPosts(db);
    const post = yield* queriesMod.findPostBySlug(db, {slug: 'hello'});
    return {posts, post};
  });

  await expect(Effect.runPromise(program as Effect.Effect<any, unknown, never>)).resolves.toMatchObject({
    posts: [{id: 1, slug: 'hello', title: 'Hello'}],
    post: {id: 1, slug: 'hello', title: 'Hello'},
  });
});
...
function rewriteBareImports(source: string): string {
  const mapping: Record<string, string> = {
    ...
    'effect/Context': pathToFileURL(path.join(packageRoot, 'node_modules', 'effect', 'dist', 'esm', 'Context.js')).href,
    'effect/Layer': pathToFileURL(path.join(packageRoot, 'node_modules', 'effect', 'dist', 'esm', 'Layer.js')).href,
  };
