...
test('generated Effect service can be provided with a client and yielded', async () => {
  await using project = await createRuntimeFixture({
  ...
  const mod = await project.importTranspiledModule<any>('sql/.generated/effect.ts');

  using database = project.openDatabase();
  const client = createNodeSqliteClient(database.database);
  const DB = mod.SqlfuQueries.make().pipe(Effect.provide(mod.SqlfuQueries.DefaultServices(client)));

  const program = Effect.gen(function*() {
    const db = yield* DB;
    const posts = yield* Effect.promise(() => db.listPosts());
    const post = yield* Effect.promise(() => db.findPostBySlug({slug: 'hello'}));
    return {posts, post};
  });
