...
test('generate writes the source query hash into the generated wrapper header', async () => {
  ...
  const generatedModule = await project.readText('sql/.generated/list-posts.sql.ts');
  expect(generatedModule.split('\n').slice(0, 2)).toEqual([
