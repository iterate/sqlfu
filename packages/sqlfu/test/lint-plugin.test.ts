...
import {generatedWrapperFileForSqlFile, renderQuerySourceManifest} from '../src/query-identity.js';
...
function queriesManifest(entries: {sqlFile: string; sourceSql: string}[]): string {
  return renderQuerySourceManifest({
    entries: entries.map((entry) => ({
      sqlFile: entry.sqlFile,
      generatedFile: generatedWrapperFileForSqlFile(entry.sqlFile),
