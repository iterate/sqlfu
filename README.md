...
Opt in to runtime validation by setting `generate.zod: true` in your config. Wrappers then emit zod schemas as the source of truth, validate params on the way in, validate each row on the way out, and derive types via `z.infer`. See [docs/runtime-validation.md](packages/sqlfu/docs/runtime-validation.md).
