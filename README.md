...
Each one is a ~50-line adapter file — see [docs/adapters.md](packages/sqlfu/docs/adapters.md) for the full table, copy-paste snippets, and guidance on which to pick. The same generated wrappers run against any of them unchanged.

One thing sqlfu goes out of its way to preserve: **sync stays sync**. A client built on top of a synchronous driver (`better-sqlite3`, `node:sqlite`, Durable Objects) is itself synchronous — no spurious `async` creeping up your call stack. Generated query wrappers follow the same rule. See [sync stays sync](packages/sqlfu/docs/adapters.md#sync-stays-sync) for the details.
...
`sqlfu` ships a lint plugin as a sub-export (`sqlfu/lint-plugin`). It targets both ESLint and oxlint (via oxlint's alpha `jsPlugins` loader) and ships three rules plus a processor for standalone `.sql` files:
...
- **`sqlfu/format-sql-file`** (+ `sqlfu/sql-file` processor) — same formatter applied to whole `.sql` files on disk. Ships as an ESLint processor so `eslint --fix '**/*.sql'` reformats the files in place. (oxlint's jsPlugins alpha doesn't support processors yet, so this rule is ESLint-only.)
...
Or in `.oxlintrc.json` (oxlint 1.60+) — inline-SQL rules only, since oxlint can't run the processor:
